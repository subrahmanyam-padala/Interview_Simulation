...
function ResetPasswordPage() {
  ...
  const [form, setForm] = useState({
    ...
    token: location.state?.token || searchParams.get('token') || '',
    password: '',
  });
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      setError(getAuthErrorMessage(requestError, 'Reset failed'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="mx-auto flex min-h-[calc(100vh-6rem)] w-full max-w-6xl items-center justify-center">
        <form className="w-full max-w-[480px] rounded-[20px] border border-slate-200 bg-white p-10 shadow-xl shadow-slate-200/70" onSubmit={onSubmit}>
          <div className="space-y-2">
            ...
            <p className="text-base leading-7 text-slate-600">Create a new password for your account.</p>
          </div>
